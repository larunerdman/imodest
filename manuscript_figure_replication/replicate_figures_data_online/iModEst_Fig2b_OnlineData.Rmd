---
title: "iModEst_Fig2b_OnlineData"
...
```{r}
r2.added.list <- list() ### HAVE ARNAV ADD THESE FILES
# file.list <- list.files("C:/Users/larun/Desktop/Modes/Data/Clustering-output/normal-samples/")

for(cancer in names(norm.r2.dfs)){
  r2.added.list[[cancer]] <- read_remote_rds(paste0('https://imodesttool.com/api/data/download/full/performance_files/tumour-adjacent_R2_added_files/',cancer,"-var-added-list.rds"))
}
...
```{r}
## 
##    BARPLOT OF THE AVERAGE (OVER ALL MODELS) 
##      R2 ADDED BY EACH REGULATOR FOR EACH CANCER (14 CANCERS)
##
# Fig: Accuracy by regulator across cancers -- 14 cancers boxplots
theme_set(
  theme_classic(base_size = 20)
)

ggplot(test,aes(x = variable, y = value,fill = cancer)) + 
  geom_boxplot() + xlab("Regulator") + ylab("R2 added") + ylim(c(-1,1)) + 
  theme(axis.text=element_text(size=12),
        axis.title=element_text(size=14,face="bold")) + 
  scale_fill_manual(values = tum.cols[names(tum.cols) %in% names(norm.r2.dfs)]) + 
